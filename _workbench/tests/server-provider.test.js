...
test('failed Codex turn cannot mark an existing draft as newly completed', { skip: process.platform === 'win32' }, async t => {
  ...
  fs.mkdirSync(work);
  for (const file of ['server.js', 'codex-provider.js', 'claude-provider.js', 'settings.js', 'questions.js', 'review.js', 'rules.js']) fs.copyFileSync(path.join(__dirname, '..', file), path.join(work, file));

...
def ignore(directory,names):
    return [n for n in names if n in {'node_modules','__pycache__','.git','token','auth.json','.env','settings.local.json','settings.json','settings.json.tmp','.rule-approvals.json','run.events.jsonl','runs','_proposed','mine-check.md'} or n.endswith(('.log','.pyc')) or (n.startswith('.env.') and n!='.env.example')]
...
w.mkdir()
for name in ['server.js','codex-provider.js','claude-provider.js','settings.js','questions.js','review.js','rules.js','shard_plan.py','merge_cards.py','verify_cards.py','facts_candidates.py','convert_questions.js','make_xlsx.py','read_xlsx.py','index_doc.py','recompute.py','package.json','package-lock.json','requirements.txt','README.md','export_demo.sh','public','tests']:

...
def ignore(directory,names):
    return [n for n in names if n in {'node_modules','__pycache__','.git','token','auth.json','.env','settings.local.json','settings.json','settings.json.tmp','.rule-approvals.json','run.events.jsonl','runs','_proposed','mine-check.md','conversations','retrieval','_render','_private_memory','.tessdata'} or n.endswith(('.log','.pyc')) or (n.startswith('.env.') and n!='.env.example')]
...
w.mkdir()
for name in ['server.js','document-tree.js','codex-provider.js','claude-provider.js','settings.js','questions.js','review.js','rules.js','shard_plan.py','merge_cards.py','verify_cards.py','facts_candidates.py','convert_questions.js','project-chat.js','memory-store.js','memory-search.py','retrieval.py','enrich-document.js','document_enrichment.py','setup_ocr.py','CHAT_DESIGN.md','MEMORY_DESIGN.md','UI_DESIGN.md','make_xlsx.py','read_xlsx.py','index_doc.py','office_convert.py','render_document.py','search.py','recompute.py','package.json','package-lock.json','requirements.txt','README.md','export_demo.sh','public','tests']:

...
### 階段 2：統整校對
- 使用者交出他的版本（工作台上傳至 `_analysis/inbox/`）後做三類 diff：共識（語意相同即算，措辭合併取較佳）／只有 Codex（使用者勾選決定，砍題附原因）／只有使用者（**一律保存、來源標「你」、Reviewer 可給建議，由人確認；記錄為盲區資料**）。合併輸出 `draft_RN_merged.md`，含「來源」欄（共識／Codex／你）。
